...
# Find the HASH_MISMATCH(manifest.json expected=... got=...) line
$idx = -1
for($i=0; $i -lt $lines.Length; $i++){
  $ln = $lines[$i]
  if ($ln -match 'HASH_MISMATCH:' -and $ln -match 'manifest\.json' -and $ln -match 'expected=' -and $ln -match 'got=') {
    $idx = $i
    break
  }
}
if ($idx -lt 0) { Die "PATCH_FAILED: could not find HASH_MISMATCH line containing manifest.json + expected= + got=" }

# Find nearest preceding variable assignment that mentions sha256sums.txt to learn the real sums var name
# Example matches: $sums = Join-Path ... "sha256sums.txt"
$sumsVar = $null
for($j=$idx; $j -ge 0 -and ($idx - $j) -le 120; $j--){
  $ln = $lines[$j]
  $m = [regex]::Match($ln, '^\s*\$([A-Za-z_][A-Za-z0-9_]*)\s*=\s*.*sha256sums\.txt')
  if ($m.Success) { $sumsVar = '$' + $m.Groups[1].Value; break }
}
if (-not $sumsVar) { Die "PATCH_FAILED: could not find a sha256sums.txt assignment within 120 lines before HASH_MISMATCH" }

# Also capture indentation for neat insertion
$ind = ([regex]::Match($lines[$idx], '^\s*')).Value

# Build replacement block (no [void] left-hand assignments; PS5.1-safe)
$orig = $lines[$idx]

$block = New-Object System.Collections.Generic.List[string]
$block.Add($ind + '# --- BEGIN manifest sha256sums producer-repair v6 ---') | Out-Null
...
$block.Add($ind + '    Write-Host "WARN_HASH_MISMATCH_MANIFEST_BUILDER_REPAIR_V6" -ForegroundColor Yellow') | Out-Null
...
$block.Add($ind + '}') | Out-Null
$block.Add($ind + '# --- END manifest sha256sums producer-repair v6 ---') | Out-Null
$block.Add($orig) | Out-Null

# Splice: replace the single mismatch line with the block
...
$bak = $TargetPath + ".bak_manifest_selfheal_v6_" + (Get-Date).ToUniversalTime().ToString("yyyyMMdd_HHmmss")
...
Write-Host ("PATCH_OK_MANIFEST_HASH_SELFHEAL_V6: " + $TargetPath) -ForegroundColor Green
