...
if(-not $did){
  # IMPORTANT: no $it in a double-quoted string under StrictMode
  Die 'PATCH_FAILED: anchor Copy-Item line not found (Copy-Item -LiteralPath $it.FullName -Destination $target -Force).'
}

$dst = ($out -join "`n")
ParseGateUtf8NoBomText $dst $TargetPath

$bak = $TargetPath + ".bak_copyitem_parent_inline_v16_" + (Get-Date).ToUniversalTime().ToString("yyyyMMdd_HHmmss")
...
Write-Host ("PATCH_OK_COPYITEM_PARENT_INLINE_V16: " + $TargetPath) -ForegroundColor Green
