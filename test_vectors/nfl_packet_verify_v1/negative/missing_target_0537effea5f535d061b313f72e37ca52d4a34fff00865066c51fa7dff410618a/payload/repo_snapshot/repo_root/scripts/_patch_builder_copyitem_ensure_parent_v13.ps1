...
for($i=0;$i -lt $lines.Count;$i++){
  ...
  # Match Copy-Item line that ends at -Destination $target (no -Force on same line)
  # and whose NEXT line is "-Force" (possibly indented)
  $isCopyLine = ($ln -match '^\s*Copy-Item\s+-LiteralPath\s+\$it\.FullName\s+-Destination\s+\$target\s*$')
  if($isCopyLine){
    $next = $null
    if(($i + 1) -lt $lines.Count){ $next = $lines[$i+1] }

    if($null -ne $next -and $next -match '^\s*-Force\b'){
      $indent = ""
      $m = [regex]::Match($ln,'^(\s*)')
  ...
  # Also keep the v12 single-line case (just in case there are more)
...
if(-not $did){ Die "PATCH_FAILED: did not find a Copy-Item `$it.FullName -> `$target site (single-line or 2-line -Force form)." }

$dst = ($out -join "`n")
ParseGateUtf8NoBomText $dst $TargetPath

$bak = $TargetPath + ".bak_copyitem_ensure_parent_v13_" + (Get-Date).ToUniversalTime().ToString("yyyyMMdd_HHmmss")
...
Write-Host ("PATCH_OK_COPYITEM_ENSURE_PARENT_V13: " + $TargetPath) -ForegroundColor Green
