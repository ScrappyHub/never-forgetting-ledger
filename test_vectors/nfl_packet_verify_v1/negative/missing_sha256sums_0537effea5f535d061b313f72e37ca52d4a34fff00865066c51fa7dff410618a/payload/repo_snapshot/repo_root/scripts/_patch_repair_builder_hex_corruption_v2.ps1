...
# Anchor on known corruption fragment variants (do NOT require parsing first)
$frags = @(
  '.ToString("x2") }) -join "")',
  '.ToString("x2")}) -join "")',
  '.ToString("x2") }) -join ""',
...
$fragUsed = ""
foreach ($f in $frags) {
  $i = $src.IndexOf($f, [System.StringComparison]::Ordinal)
  if ($i -ge 0) { $fragIdx = $i; $fragUsed = $f; break }
}
if ($fragIdx -lt 0) { Die ("FRAGMENT_NOT_FOUND: expected ToString(x2) corruption fragment in " + $TargetPath) }

# Find containing function: nearest "function " above, next "function " below
$funcTok = "function "
...
# Replacement: same function name; literal $Bytes (no expansion); PS5.1-safe; no pipelines
$fixed = @(
...
Write-Host ("PATCH_OK_BUILDER_HEX_CORRUPTION_FIXED_V2: " + $TargetPath + "  func=" + $funcName + "  frag=" + $fragUsed) -ForegroundColor Green
