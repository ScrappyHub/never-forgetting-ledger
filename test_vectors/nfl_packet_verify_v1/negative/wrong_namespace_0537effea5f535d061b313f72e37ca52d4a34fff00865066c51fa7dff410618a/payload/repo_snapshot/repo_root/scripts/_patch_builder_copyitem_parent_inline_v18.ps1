...
for($i=0;$i -lt $lines.Count;$i++){
  ...
  if($looksLikeCopyItem){
    # If guard already present immediately above, do nothing
...
if($inserted -le 0){
  Die 'PATCH_FAILED: no matching Copy-Item windows found to patch (Copy-Item + $it.FullName + -Destination + $target).'
}

$dst = ($out -join "`n")
ParseGateUtf8NoBomText $dst $TargetPath

$bak = $TargetPath + ".bak_copyitem_parent_inline_v18_" + (Get-Date).ToUniversalTime().ToString("yyyyMMdd_HHmmss")
...
Write-Host ("PATCH_OK_COPYITEM_PARENT_INLINE_V18: " + $TargetPath + "  inserted=" + $inserted) -ForegroundColor Green
