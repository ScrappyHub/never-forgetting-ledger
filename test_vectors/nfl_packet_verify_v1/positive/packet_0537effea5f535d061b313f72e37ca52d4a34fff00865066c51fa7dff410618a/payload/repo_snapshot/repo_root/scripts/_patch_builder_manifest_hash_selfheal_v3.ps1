...
$sumVar = ""
$sumRx = New-Object System.Text.RegularExpressions.Regex('(?im)^\s*(\$\w+)\s*=\s*.*sha256sums\.txt')
...
if ($sumVar -ne "") {
  ...
  [void]$repl.Add("    $mf = Join-Path $root ""manifest.json""")
  [void]$repl.Add("    $mh = (Get-FileHash -Algorithm SHA256 -LiteralPath $mf).Hash.ToLowerInvariant()")
...
$bak = $TargetPath + ".bak_manifest_selfheal_v3_" + (Get-Date).ToUniversalTime().ToString("yyyyMMdd_HHmmss")
...
Write-Host ("PATCH_OK_MANIFEST_HASH_SELFHEAL_V3: " + $TargetPath + "  func=" + $funcName + "  relVar=" + $relVar + "  sumVar=" + $sumVar) -ForegroundColor Green
