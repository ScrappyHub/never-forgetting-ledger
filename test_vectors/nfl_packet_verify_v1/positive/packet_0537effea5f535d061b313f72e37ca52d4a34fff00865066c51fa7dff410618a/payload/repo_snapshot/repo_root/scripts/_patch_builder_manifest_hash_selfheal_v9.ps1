...
function ParseGateText([string]$Text,[string]$Path){
  # IMPORTANT: do NOT pipe a [void] expression (PS5.1 throws "Argument type cannot be System.Void.")
...
# Idempotency: if marker already present, stop cleanly
$marker = 'BEGIN manifest sha256sums producer-repair v9'
foreach($ln in $lines){
  if ($ln -like ("*"+$marker+"*")) {
    Write-Host ("ALREADY_PATCHED_V9: " + $TargetPath) -ForegroundColor Yellow
    return
  }
}
...
$repair.Add($ind + '# --- BEGIN manifest sha256sums producer-repair v9 ---') | Out-Null
$repair.Add($ind + 'try {') | Out-Null
$repair.Add($ind + '  $rel = $null') | Out-Null
$repair.Add($ind + '  foreach($n in @("rel","relVar","relPath","rel_file","relFile","relname","relName","relative","relativePath","relp","rp")){') | Out-Null
...
$repair.Add($ind + '    Write-Host "WARN_HASH_MISMATCH_MANIFEST_BUILDER_REPAIR_V9" -ForegroundColor Yellow') | Out-Null
...
$repair.Add($ind + '}') | Out-Null
$repair.Add($ind + '# --- END manifest sha256sums producer-repair v9 ---') | Out-Null
...
$bak = $TargetPath + ".bak_manifest_selfheal_v9_" + (Get-Date).ToUniversalTime().ToString("yyyyMMdd_HHmmss")
...
Write-Host ("PATCH_OK_MANIFEST_HASH_SELFHEAL_V9: " + $TargetPath) -ForegroundColor Green
