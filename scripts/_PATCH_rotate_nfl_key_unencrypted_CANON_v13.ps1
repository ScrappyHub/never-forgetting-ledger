...
try {
  ...
  $wslPriv    = WinPath-ToWslLoose $Priv
  $wslPayload = WinPath-ToWslLoose $payload
  $wslAllowed = WinPath-ToWslLoose $allowed
  $wslRev     = WinPath-ToWslLoose $rev

  Write-Output ("RUN: wsl -e ssh-keygen -Y sign ...")
  ...
  # Grab default sig and normalize
  ...
  # Now it's safe to convert the sig path for verify
  $wslSig = WinPath-ToWslLoose $sig
  ...
  Write-Output ("RUN: wsl -e ssh-keygen -Y verify ...")
