...
# ----------------------------
# 0) Rewrite sign_file_v1.ps1 (SIGN LITERAL BYTES; no normalization)
# ----------------------------
...
# Canonical NFL key location (repo-managed). This MUST be the key that corresponds to proofs\\keys\\nfl-local_ed25519.pub
`$KeyPriv = Join-Path `$Root "proofs\keys\nfl-local_ed25519"

# Canonical signer identity (matches allowed_signers principal)
...
# Deterministic: remove old sig (avoid prompts / stale sig confusion)
if (Test-Path -LiteralPath `$OutSig -PathType Leaf) { Remove-Item -LiteralPath `$OutSig -Force }

# Sign LITERAL BYTES (ssh-keygen reads the file directly)
Write-Host ("Signing file " + `$File)

# Use ssh-keygen -Y sign with explicit identity (-I) so verify can bind the principal deterministically.
Invoke-OpenSshKeygen -Argv @("-Y","sign","-f",`$KeyPriv,"-I",`$Principal,"-n",`$Namespace,"-s",`$OutSig,`$File) -TimeoutSeconds `$TimeoutSeconds | Out-Null
...
# ----------------------------
# 1) Rewrite verify_sig_v1.ps1 (debug SHA + real empty revocation file)
# ----------------------------
...
# ----------------------------
# 2) Rewrite debug_verify_find_principals_v1.ps1 (include SHA)
# ----------------------------
