...
# KEYGEN via cmd, force non-interactive by piping a newline
# Important cmd quoting:
#   - whole thing passed to cmd; use "" for embedded quotes.
#   - -N "" expressed as -N """" in cmd.
...
if (-not [string]::IsNullOrWhiteSpace($r.Stderr)) { Write-Output "KEYGEN_STDERR:"; Write-Output $r.Stderr }
if (-not [string]::IsNullOrWhiteSpace($r.Stdout)) { Write-Output "KEYGEN_STDOUT:"; Write-Output $r.Stdout }
if ($r.ExitCode -ne 0) { Die ("KEYGEN_FAIL (exit " + $r.ExitCode + ")`nCMD: " + $r.Cmd) }
...
$cmdDerive = 'echo.| ""' + $ssh + '"" -y -f ""' + $Priv + '""'
Write-Output ("RUN: " + $cmdDerive)

$ry = Invoke-CmdSync -CmdLine $cmdDerive -TimeoutSeconds 10
Write-Output ("DERIVE_EXIT: " + $ry.ExitCode)
if ($ry.ExitCode -ne 0) { Die ("DERIVE_PUB_FAIL (exit " + $ry.ExitCode + ")`nCMD: " + $ry.Cmd + "`nSTDERR:`n" + $ry.Stderr + "`nSTDOUT:`n" + $ry.Stdout) }
