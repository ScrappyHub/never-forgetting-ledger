...
function WriteSha256Sums([string]$Root,[string]$OutPath,[string[]]$FilesAbs){
  ...
  foreach($fp in $FilesAbs){
    if(-not (Test-Path -LiteralPath $fp -PathType Leaf)){ throw ("SHA256SUMS_MISSING_FILE: " + $fp) }
    $hex = Sha256Hex $fp
    $rel = RelPath $Root $fp
    [void]$rows.Add(($hex + "  " + $rel))
  }
  WriteUtf8NoBomLf $OutPath ((@($rows.ToArray()) -join "`n") + "`n")
}
...
function AssertAnyTokenExact([string]$Text,[string[]]$ExactTokens,[string]$FailToken){
  $t = NormalizeLf $Text
...
# ---- runner v2 (overwrite, known-good, narrow parse-gate) ----
...
[void]$R.Add('function WriteUtf8NoBomLf([string]$Path,[string]$Text){ $dir=Split-Path -Parent $Path; if($dir){ EnsureDir $dir }; [System.IO.File]::WriteAllBytes($Path,(Utf8NoBom).GetBytes((NormalizeLf $Text))); if(-not (Test-Path -LiteralPath $Path -PathType Leaf)){ throw ("WRITE_FAILED: " + $Path) } }')
...
[void]$R.Add('')
[void]$R.Add('$must=@((Join-Path $ScriptsDir "verify_packet_v1.ps1"),(Join-Path $ScriptsDir "selftest_verify_packet_v1.ps1"),(Join-Path $ScriptsDir "selftest_vectors_v1.ps1"),(Join-Path $ScriptsDir "_selftest_nfl_tier0_v2.ps1"))')
