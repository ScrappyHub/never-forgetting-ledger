...
# HARD DELETE targets to guarantee NO overwrite prompt
foreach($p in @($Priv,$Pub)){
  if (Test-Path -LiteralPath $p -PathType Leaf) {
...
# Generate new UNENCRYPTED key (no prompts)
...
# Prove non-interactive access (this must NOT hang)
...
Write-Output "OK: rotated NFL signing key to UNENCRYPTED repo-managed key (next: update allowed_signers + resign)"
