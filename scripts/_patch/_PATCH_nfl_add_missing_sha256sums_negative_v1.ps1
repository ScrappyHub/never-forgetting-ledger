...
function Die([string]$m){ throw ("NFL_PATCH_MISSING_SHA256SUMS_FAIL: " + $m) }
...
if($vtxt -notlike "*MISSING_SHA256SUMS*"){
  $old = @'
...
EnsureDir $negRoot
$newName   = "missing_sha256sums"
$newExpect = "MISSING_SHA256SUMS"
$newRoot   = Join-Path $negRoot ($newName + "_" + $packetId)
CopyDirDeterministic $posRoot $newRoot
$sums = Join-Path $newRoot "sha256sums.txt"
if(Test-Path -LiteralPath $sums -PathType Leaf){ Remove-Item -LiteralPath $sums -Force }
...
$orderedNames = @("tamper_payload","missing_sig","wrong_namespace","missing_sha256sums")
...
# 3) patch runner expected set from 3 -> 4 tokens
$rtxt = NormalizeLf (ReadUtf8 $Runner)
$oldExpected3 = '$expected=@("NFL_VERIFY_FAIL:SHA256_MISMATCH","NFL_VERIFY_FAIL:MISSING_SIG","NFL_VERIFY_FAIL:SIG_VERIFY_FAIL")'
$newExpected4 = '$expected=@("NFL_VERIFY_FAIL:SHA256_MISMATCH","NFL_VERIFY_FAIL:MISSING_SIG","NFL_VERIFY_FAIL:SIG_VERIFY_FAIL","NFL_VERIFY_FAIL:MISSING_SHA256SUMS")'
if($rtxt -like ("*" + $oldExpected3 + "*")){
  $rtxt = $rtxt.Replace($oldExpected3,$newExpected4)
} elseif($rtxt -notlike ("*" + $newExpected4 + "*")){
