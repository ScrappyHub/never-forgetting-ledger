...
function Die([string]$m){ throw ("NFL_PATCH_BAD_SHA256SUM_LINE_FAIL: " + $m) }
...
if($vtxt -notlike "*BAD_SHA256SUM_LINE*"){
  $old = @'
...
$orderedNames = @("tamper_payload","missing_sig","wrong_namespace","missing_sha256sums","bad_sha256sum_line")
...
# 3) patch runner expected set from 4 -> 5 tokens
$rtxt = NormalizeLf (ReadUtf8 $Runner)
$oldExpected4 = '$expected=@("NFL_VERIFY_FAIL:SHA256_MISMATCH","NFL_VERIFY_FAIL:MISSING_SIG","NFL_VERIFY_FAIL:SIG_VERIFY_FAIL","NFL_VERIFY_FAIL:MISSING_SHA256SUMS")'
$newExpected5 = '$expected=@("NFL_VERIFY_FAIL:SHA256_MISMATCH","NFL_VERIFY_FAIL:MISSING_SIG","NFL_VERIFY_FAIL:SIG_VERIFY_FAIL","NFL_VERIFY_FAIL:MISSING_SHA256SUMS","NFL_VERIFY_FAIL:BAD_SHA256SUM_LINE")'
if($rtxt -like ("*" + $oldExpected4 + "*")){
  $rtxt = $rtxt.Replace($oldExpected4,$newExpected5)
} elseif($rtxt -notlike ("*" + $newExpected5 + "*")){
