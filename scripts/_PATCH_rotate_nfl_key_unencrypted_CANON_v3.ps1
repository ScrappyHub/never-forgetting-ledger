...
function Quote-Arg([string]$x){
  if ($null -eq $x) { return '""' }        # preserve null as empty-string arg
  if ($x -eq "")    { return '""' }        # CRITICAL: preserve empty-string arg (for -N "")
...
# Generate new UNENCRYPTED key: -N "" MUST survive argv quoting
