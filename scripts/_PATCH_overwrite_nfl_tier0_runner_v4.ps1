...
function WriteUtf8NoBomLf([string]$Path,[string]$Text){
  ...
  $enc = Utf8NoBom
  [System.IO.File]::WriteAllBytes($Path, $enc.GetBytes((NormalizeLf $Text)))
...
if(Test-Path -LiteralPath $OutRunner -PathType Leaf){
  $bak = $OutRunner + ".bak_" + (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssZ")
  Copy-Item -LiteralPath $OutRunner -Destination $bak -Force
}
