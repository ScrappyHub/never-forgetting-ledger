...
try {
  ...
  # Build a WSL workdir and copy files into ext4 so chmod works
  $workId = ("nfl_keytest_" + [Guid]::NewGuid().ToString("N"))
  $work   = ("$HOME/.nfl-keytest/" + $workId)

  Write-Output ("WSL_WORKDIR: " + $work)

  $cmdPrep = "set -e; umask 077; mkdir -p `"$work`"; cp `"$wslPrivSrc`" `"$work/key`"; cp `"$wslPrivSrc.pub`" `"$work/key.pub`"; cp `"$wslPayloadSrc`" `"$work/payload.txt`"; cp `"$wslAllowedSrc`" `"$work/allowed_signers`"; cp `"$wslRevSrc`" `"$work/revoked_keys`"; chmod 600 `"$work/key`"; chmod 644 `"$work/key.pub`" `"$work/payload.txt`" `"$work/allowed_signers`" `"$work/revoked_keys`""
  Write-Output "RUN: wsl -e sh -lc <prep>"
  $rp = Invoke-WslSync -Argv @("sh","-lc",$cmdPrep) -TimeoutSeconds 10
  if ($rp.ExitCode -ne 0) { Die ("WSL_PREP_FAIL`nSTDERR:`n" + $rp.Stderr + "`nSTDOUT:`n" + $rp.Stdout) }

  $cmdSign = "set -e; cd `"$work`"; ssh-keygen -Y sign -f `"$work/key`" -I `"$TestPrincipal`" -n `"$TestNamespace`" `"$work/payload.txt`""
  Write-Output "RUN: wsl -e sh -lc <sign>"
  $rs = Invoke-WslSync -Argv @("sh","-lc",$cmdSign) -TimeoutSeconds 10
  if ($rs.ExitCode -ne 0) { Die ("SIGN_FAIL`nSTDERR:`n" + $rs.Stderr + "`nSTDOUT:`n" + $rs.Stdout) }

  # Copy signature back to Windows temp for hashing/proof
  $wslSig = ($work + "/payload.txt.sig")
  $cmdCopySig = "set -e; test -f `"$wslSig`"; cp `"$wslSig`" `"$wslPayloadSrc.sig`""
  Write-Output "RUN: wsl -e sh -lc <copy sig back>"
  $rc = Invoke-WslSync -Argv @("sh","-lc",$cmdCopySig) -TimeoutSeconds 10
  ...
  $cmdVerify = "set -e; cd `"$work`"; ssh-keygen -Y verify -f `"$work/allowed_signers`" -I `"$TestPrincipal`" -n `"$TestNamespace`" -s `"$work/payload.txt.sig`" -r `"$work/revoked_keys`" `"$work/payload.txt`""
  Write-Output "RUN: wsl -e sh -lc <verify>"
  $rv = Invoke-WslSync -Argv @("sh","-lc",$cmdVerify) -TimeoutSeconds 10
  if ($rv.ExitCode -ne 0) { Die ("VERIFY_FAIL`nSTDERR:`n" + $rv.Stderr + "`nSTDOUT:`n" + $rv.Stdout) }

  # Cleanup workdir (best-effort)
  $cmdClean = "rm -rf `"$work`""
  [void](Invoke-WslSync -Argv @("sh","-lc",$cmdClean) -TimeoutSeconds 10)
