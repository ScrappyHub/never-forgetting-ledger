...
function AssertAnyTokenExact([string]$Text,[string[]]$ExactTokens,[string]$FailToken){ $t=NormalizeLf $Text; foreach($x in $ExactTokens){ if($t -match ("(?m)^\Q" + $x + "\E$")){ return } }; throw ($FailToken + ":NO_EXPECTED_TOKEN") }
function JsonEsc([string]$s){ if($null -eq $s){ return "" }; $bs=[string][char]92; $dq=[string][char]34; $t=$s.Replace($bs,$bs+$bs).Replace($dq,$bs+$dq); $t=($t -replace "`r",""); $t=$t.Replace("`n","\n"); return $t }
...
# ---- write runner (direct, no generator-of-generator) ----
$runnerText = @(
  "param([Parameter(Mandatory=`$true)][string]`$RepoRoot)",
  "Set-StrictMode -Version Latest",
  "`$ErrorActionPreference=`"Stop`"",
  ...
  "function NormalizeLf([string]`$t){ if(`$null -eq `$t){ return `"`" }; `$u = (`$t -replace ``"`r`n``",``"`n``") -replace ``"`r``",``"`n``"; if(-not `$u.EndsWith(`"`n`")){ `$u += `"`n`" }; return `$u }",
  "function Utf8NoBom(){ New-Object System.Text.UTF8Encoding(`$false) }",
  "function WriteUtf8NoBomLf([string]`$Path,[string]`$Text){ `$dir=Split-Path -Parent `$Path; if(`$dir){ EnsureDir `$dir }; [System.IO.File]::WriteAllBytes(`$Path,(Utf8NoBom).GetBytes((NormalizeLf `$Text))); if(-not (Test-Path -LiteralPath `$Path -PathType Leaf)){ throw (`"WRITE_FAILED: `"+`$Path) } }",
  "function Sha256Hex([string]`$Path){ (Get-FileHash -LiteralPath `$Path -Algorithm SHA256).Hash.ToLowerInvariant() }",
  "function RelPath([string]`$Root,[string]`$Full){ `$bs=[char]92; `$r=(Resolve-Path -LiteralPath `$Root).Path.TrimEnd(`$bs); `$f=(Resolve-Path -LiteralPath `$Full).Path; if(`$f.Length -lt `$r.Length){ return `$f.Replace(`$bs,[char]47) }; if(`$f.Substring(0,`$r.Length).ToLowerInvariant() -ne `$r.ToLowerInvariant()){ return `$f.Replace(`$bs,[char]47) }; `$rel=`$f.Substring(`$r.Length).TrimStart(`$bs); return `$rel.Replace(`$bs,[char]47) }",
  "function WriteSha256Sums([string]`$Root,[string]`$OutPath,[string[]]`$FilesAbs){ `$rows=New-Object System.Collections.Generic.List[string]; foreach(`$fp in `$FilesAbs){ if(-not (Test-Path -LiteralPath `$fp -PathType Leaf)){ throw (`"SHA256SUMS_MISSING_FILE: `"+`$fp) }; `$hex=Sha256Hex `$fp; `$rel=RelPath `$Root `$fp; [void]`$rows.Add((`$hex + `"  `" + `$rel)) }; WriteUtf8NoBomLf `$OutPath ((@(`$rows.ToArray()) -join `"`n`") + `"`n`") }",
  "function RunChild([string]`$PSExe,[string]`$ScriptPath,[string]`$RepoRoot,[string]`$StdOut,[string]`$StdErr){ if(Test-Path -LiteralPath `$StdOut -PathType Leaf){ Remove-Item -LiteralPath `$StdOut -Force }; if(Test-Path -LiteralPath `$StdErr -PathType Leaf){ Remove-Item -LiteralPath `$StdErr -Force }; `$psi=New-Object System.Diagnostics.ProcessStartInfo; `$psi.FileName=`$PSExe; `$psi.Arguments=(`"-NoProfile -NonInteractive -ExecutionPolicy Bypass -File ```"{0}```" -RepoRoot ```"{1}```"`" -f `$ScriptPath,`$RepoRoot); `$psi.WorkingDirectory=`$RepoRoot; `$psi.UseShellExecute=`$false; `$psi.RedirectStandardOutput=`$true; `$psi.RedirectStandardError=`$true; `$psi.CreateNoWindow=`$true; `$pp=New-Object System.Diagnostics.Process; `$pp.StartInfo=`$psi; `$null=`$pp.Start(); `$so=`$pp.StandardOutput.ReadToEnd(); `$se=`$pp.StandardError.ReadToEnd(); `$pp.WaitForExit(); WriteUtf8NoBomLf `$StdOut `$so; WriteUtf8NoBomLf `$StdErr `$se; return @{ exit=[int]`$pp.ExitCode; stdout=`$so; stderr=`$se } }",
  ...
  "AssertContains `$r1.stdout `"VERIFY_SELFTEST_OK`" `"SELFTEST_VERIFY_PACKET_BAD_STDOUT`"",
  ...
  "AssertContains `$r2.stdout `"VECTORS_SELFTEST_OK`" `"SELFTEST_VECTORS_BAD_STDOUT`"",
  ...
  "`$line=(`"{{`" + `"`"type`"`":`"`"nfl.tier0.selftest.v1`"`",`" + `"`"utc`"`":`"`"`" + (JsonEsc `$stamp) + `"`"`",`" + `"`"ok`"`":true,`" + `"`"repo_root`"`":`"`"`" + (JsonEsc `$RepoRoot) + `"`"`",`" + `"`"bundle_dir`"`":`"`"`" + (JsonEsc `$bundle) + `"`"`",`" + `"`"selftest_verify_exit`"`":`" + `$r1.exit + `",`" + `"`"selftest_vectors_exit`"`":`" + `$r2.exit + `"`"}}`")",
...
$runnerBody = (NormalizeLf ((@($runnerText) -join "`n") + "`n"))
WriteUtf8NoBomLf $Runner $runnerBody
