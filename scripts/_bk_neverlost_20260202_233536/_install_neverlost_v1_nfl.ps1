...
$OpenSshKeygen = Join-Path $env:WINDIR "System32\OpenSSH\ssh-keygen.exe"
if (-not (Test-Path -LiteralPath $OpenSshKeygen)) { throw ("Missing Windows OpenSSH ssh-keygen: " + $OpenSshKeygen) }
Set-StrictMode -Version Latest


function Invoke-OpenSshKeygen([string[]]$Args) {
  ...
  if ($p.ExitCode -ne 0) { throw ("ssh-keygen failed (exit " + $p.ExitCode + "): " + ($Args -join " ")) }
}
$Root = "C:\dev\nfl"
...
$lib += '  $defaultSig = $FilePath + ".sig"'
  # --- NeverLost dev keygen prelude (self-contained) ---
  $KeysDir = Join-Path $Root "proofs\keys"
  New-Item -ItemType Directory -Force -Path $KeysDir | Out-Null
  if (-not (Get-Variable -Name AuthorityName -Scope Script -ErrorAction SilentlyContinue)) { $script:AuthorityName = "nfl-local" }
  $KeyPath = Join-Path $KeysDir ($script:AuthorityName + "_ed25519")
  $PubKey  = $KeyPath + ".pub"
  $Comment = "neverlost-dev " + $script:AuthorityName
  if (Test-Path -LiteralPath $KeyPath) { Remove-Item -LiteralPath $KeyPath -Force }
  if (Test-Path -LiteralPath $PubKey)  { Remove-Item -LiteralPath $PubKey  -Force }
  # --- end prelude ---
Invoke-OpenSshKeygen @("-t","ed25519","-f",$KeyPath,"-N","", "-C", $Comment)
...
if (-not (Test-Path -LiteralPath $PrivKey) -or -not (Test-Path -LiteralPath $PubKey)) {
  ...
  $KeysDir = Join-Path $Root "proofs\keys"
  New-Item -ItemType Directory -Force -Path $KeysDir | Out-Null
  $Comment = "neverlost-dev " + $AuthorityName
  if (Test-Path -LiteralPath $PrivKey) { Remove-Item -LiteralPath $PrivKey -Force }
  if (Test-Path -LiteralPath $PubKey)  { Remove-Item -LiteralPath $PubKey  -Force }
Invoke-OpenSshKeygen @("-t","ed25519","-f",$PrivKey,"-N","", "-C", $Comment)
}
