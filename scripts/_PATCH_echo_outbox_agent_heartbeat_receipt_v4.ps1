...
if ($raw -match "ECHO_OUTBOX_AGENT_HEARTBEAT_RECEIPT_V4") { Write-Host "SKIP: heartbeat already present" -ForegroundColor Yellow; return }

# Insert immediately BEFORE final tick-ok message (so it always runs, even with 0 packets).
$pat = "(?m)^[\t ]*Write-Host\s+`"ECHO_OUTBOX_AGENT_TICK_OK`""
$m = [regex]::Match($raw, $pat)
if (-not $m.Success) { Die "PATCH_V4: anchor not found: Write-Host `"`"ECHO_OUTBOX_AGENT_TICK_OK`"`"" }
$pos = $m.Index

$ins = New-Object System.Collections.Generic.List[string]
[void]$ins.Add("")
[void]$ins.Add("# ECHO_OUTBOX_AGENT_HEARTBEAT_RECEIPT_V4")
[void]$ins.Add("try {")
...
[void]$ins.Add('    $j = "{""ts"":""" + (NowIso) + """,""agent"":""echo_outbox_agent.v1"",""event"":""heartbeat"",""status"":""ok"",""config_path"":""" + (JsonEscape $ConfigPath) + """,""receipt_path"":""" + (JsonEscape $ReceiptLogPath) + """}"' )
...
Write-Host ("PATCH_OK: heartbeat inserted into " + $AgentPath) -ForegroundColor Green
$raw2 = Get-Content -Raw -LiteralPath $AgentPath -Encoding UTF8
if ($raw2 -notmatch "ECHO_OUTBOX_AGENT_HEARTBEAT_RECEIPT_V4") { Die "PATCH_V4_FAILED: marker not found after write" }
Write-Host "PATCH_PROOF_OK: marker present" -ForegroundColor Green
