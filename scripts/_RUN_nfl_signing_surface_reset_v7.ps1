...
function Patch-FileRegexOptional_NoParse([string]$Path,[string]$Label,[string]$Pattern,[string]$Replacement){
  ...
  Write-Output ("PATCHED(best-effort): " + $Path + " [" + $Label + "]")
  return $true
}
...
# 1) Patch installer drift (BEST-EFFORT; never blocks)
$Installer = Join-Path $ScriptsDir "_install_neverlost_v1_nfl.ps1"
try {
  [void](Patch-FileRegexOptional_NoParse -Path $Installer -Label "installer_keyname_script_scope" -Pattern '\(\s*\$script:AuthorityName\s*\+\s*"_ed25519"\s*\)' -Replacement '"nfl-local_ed25519"')
  [void](Patch-FileRegexOptional_NoParse -Path $Installer -Label "installer_keyname_nonscript_scope" -Pattern '\(\s*\$AuthorityName\s*\+\s*"_ed25519"\s*\)' -Replacement '"nfl-local_ed25519"')
} catch {
  Write-Output ("WARN: installer patch skipped: " + $_.Exception.Message)
}
...
[void]$sign.Add('function Quote-Arg([string]$x){')
[void]$sign.Add('  if ($null -eq $x) { return ''""'' }')
[void]$sign.Add('  if ($x -eq "")    { return ''""'' }')
[void]$sign.Add('  if ($x -match ''[\s\x22]'') { return '"' + ($x -replace '"','\"') + '"' }')
[void]$sign.Add('  return $x')
[void]$sign.Add('}')
[void]$sign.Add('function Invoke-OpenSshKeygenPSI([string[]]$Argv,[int]$TimeoutSeconds){')
[void]$sign.Add('  if ($TimeoutSeconds -lt 1) { Die "TimeoutSeconds must be >= 1" }')
[void]$sign.Add('  $ssh = (Get-Command ssh-keygen.exe -ErrorAction Stop).Source')
[void]$sign.Add('  $parts = New-Object System.Collections.Generic.List[string]')
[void]$sign.Add('  foreach($a in @(@($Argv))){ [void]$parts.Add((Quote-Arg $a)) }')
[void]$sign.Add('  $argStr = ($parts.ToArray() -join " ")')
[void]$sign.Add('  $psi = New-Object System.Diagnostics.ProcessStartInfo')
[void]$sign.Add('  $psi.FileName = $ssh; $psi.Arguments = $argStr')
[void]$sign.Add('  $psi.UseShellExecute = $false')
[void]$sign.Add('  $psi.RedirectStandardOutput = $true; $psi.RedirectStandardError = $true; $psi.RedirectStandardInput = $true')
[void]$sign.Add('  $psi.CreateNoWindow = $true')
[void]$sign.Add('  $p = New-Object System.Diagnostics.Process; $p.StartInfo = $psi')
[void]$sign.Add('  if(-not $p.Start()){ Die "Failed to start ssh-keygen" }')
[void]$sign.Add('  try { $p.StandardInput.Close() } catch { }')
[void]$sign.Add('  $ok = $p.WaitForExit($TimeoutSeconds * 1000)')
[void]$sign.Add('  if(-not $ok){ try { $p.Kill() | Out-Null } catch { } ; Die ("ssh-keygen timeout after " + $TimeoutSeconds + "s`nARGS: " + $argStr) }')
[void]$sign.Add('  $stdout=""; $stderr=""')
[void]$sign.Add('  try { $stdout = $p.StandardOutput.ReadToEnd() } catch { $stdout = "" }')
[void]$sign.Add('  try { $stderr = $p.StandardError.ReadToEnd()  } catch { $stderr = "" }')
[void]$sign.Add('  $stdout = ($stdout -replace "`r`n","`n" -replace "`r","`n").TrimEnd()')
[void]$sign.Add('  $stderr = ($stderr -replace "`r`n","`n" -replace "`r","`n").TrimEnd()')
[void]$sign.Add('  if($p.ExitCode -ne 0){ Die ("ssh-keygen failed (exit " + $p.ExitCode + ")`nARGS: " + $argStr + "`nSTDERR:`n" + $stderr + "`nSTDOUT:`n" + $stdout) }')
[void]$sign.Add('  return $true')
[void]$sign.Add('}')
...
[void]$ver.Add('function Quote-Arg([string]$x){ if ($null -eq $x) { return ''""'' } if ($x -eq "") { return ''""'' } if ($x -match ''[\s\x22]'') { return '"' + ($x -replace '"','\"') + '"' } return $x }')
