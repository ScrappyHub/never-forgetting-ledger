...
[void]$R.Add('function RelPath([string]$Root,[string]$Full){ $bs=[char]92; $r=(Resolve-Path -LiteralPath $Root).Path.TrimEnd($bs); $f=(Resolve-Path -LiteralPath $Full).Path; if($f.Length -lt $r.Length){ return $f.Replace($bs,[char]47) }; if($f.Substring(0,$r.Length).ToLowerInvariant() -ne $r.ToLowerInvariant()){ return $f.Replace($bs,[char]47) }; $rel=$f.Substring($r.Length).TrimStart($bs); return $rel.Replace($bs,[char]47) }')
...
[void]$R.Add('function JsonEsc([string]$s){ if($null -eq $s){ return "" }; $bs=[string][char]92; $dq=[string][char]34; $t=$s.Replace($bs,$bs+$bs).Replace($dq,$bs+$dq); $t=($t -replace "`r",""); $t=$t.Replace("`n","\n"); return $t }')
[void]$R.Add('')
