...
# ----------------------------
# 2) Self-test: sign+verify in WSL (use /tmp on ext4)
# ----------------------------
...
try {
  ...
  $cmdPrep = "set -e; umask 077; WORK=`"$work`"; rm -rf `"`$WORK`"; mkdir -p `"`$WORK`"; " +
  ...
  $rp = Invoke-WslSync -Argv @("sh","-lc",$cmdPrep) -TimeoutSeconds 20
  ...
  $cmdSign = "set -e; WORK=`"$work`"; cd `"`$WORK`"; ssh-keygen -Y sign -f `"`$WORK/key`" -I `"$TestPrincipal`" -n `"$TestNamespace`" `"`$WORK/payload.txt`"; echo SIGN_OK"
  Write-Output "RUN: wsl -e sh -lc <sign>"
  $rs = Invoke-WslSync -Argv @("sh","-lc",$cmdSign) -TimeoutSeconds 20
  if ($rs.ExitCode -ne 0) { Die ("SIGN_FAIL`nSTDERR:`n" + $rs.Stderr + "`nSTDOUT:`n" + $rs.Stdout) }

  $cmdCopySig = "set -e; WORK=`"$work`"; test -f `"`$WORK/payload.txt.sig`"; cp `"`$WORK/payload.txt.sig`" `"$wslPayloadSrc.sig`"; echo COPY_SIG_OK"
  Write-Output "RUN: wsl -e sh -lc <copy sig back>"
  $rc = Invoke-WslSync -Argv @("sh","-lc",$cmdCopySig) -TimeoutSeconds 20
  ...
  $cmdVerify = "set -e; WORK=`"$work`"; cd `"`$WORK`"; ssh-keygen -Y verify -f `"`$WORK/allowed_signers`" -I `"$TestPrincipal`" -n `"$TestNamespace`" -s `"`$WORK/payload.txt.sig`" -r `"`$WORK/revoked_keys`" `"`$WORK/payload.txt`"; echo VERIFY_OK"
  Write-Output "RUN: wsl -e sh -lc <verify>"
  $rv = Invoke-WslSync -Argv @("sh","-lc",$cmdVerify) -TimeoutSeconds 20
  if ($rv.ExitCode -ne 0) { Die ("VERIFY_FAIL`nSTDERR:`n" + $rv.Stderr + "`nSTDOUT:`n" + $rv.Stdout) }

  $cmdClean = "rm -rf `"$work`""
  [void](Invoke-WslSync -Argv @("sh","-lc",$cmdClean) -TimeoutSeconds 20)

  Write-Output "PASS: key rotated + WSL /tmp sign/verify succeeded (no /mnt/c key use)"
}
finally {
