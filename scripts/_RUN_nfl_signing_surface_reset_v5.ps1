...
# Shared helpers for sign/verify scripts
$quoteFn = 'function Quote-Arg([string]$x){ if($null -eq $x){ return '''""""''' } if($x -eq ""){ return '''""""''' } if($x -match ''[\s"]''){ return ''"'' + ($x -replace ''"'',''\"'') + ''"'' } return $x }'
$invokeFn = 'function Invoke-OpenSshKeygenPSI([string[]]$Argv,[int]$TimeoutSeconds){ if($TimeoutSeconds -lt 1){ throw "TimeoutSeconds must be >= 1" } $ssh=(Get-Command ssh-keygen.exe -ErrorAction Stop).Source; $parts=New-Object System.Collections.Generic.List[string]; foreach($a in @(@($Argv))){ [void]$parts.Add((Quote-Arg $a)) }; $argStr=($parts.ToArray() -join " "); $psi=New-Object System.Diagnostics.ProcessStartInfo; $psi.FileName=$ssh; $psi.Arguments=$argStr; $psi.UseShellExecute=$false; $psi.RedirectStandardOutput=$true; $psi.RedirectStandardError=$true; $psi.RedirectStandardInput=$true; $psi.CreateNoWindow=$true; $p=New-Object System.Diagnostics.Process; $p.StartInfo=$psi; if(-not $p.Start()){ throw "Failed to start ssh-keygen" }; try{$p.StandardInput.Close()}catch{}; $ok=$p.WaitForExit($TimeoutSeconds*1000); if(-not $ok){ try{$p.Kill()|Out-Null}catch{}; throw ("ssh-keygen timeout after " + $TimeoutSeconds + "s`nARGS: " + $argStr) }; $stdout="";$stderr=""; try{$stdout=$p.StandardOutput.ReadToEnd()}catch{$stdout=""}; try{$stderr=$p.StandardError.ReadToEnd()}catch{$stderr=""}; $stdout=($stdout -replace "`r`n","`n" -replace "`r","`n").TrimEnd(); $stderr=($stderr -replace "`r`n","`n" -replace "`r","`n").TrimEnd(); if($p.ExitCode -ne 0){ throw ("ssh-keygen failed (exit " + $p.ExitCode + ")`nARGS: " + $argStr + "`nSTDERR:`n" + $stderr + "`nSTDOUT:`n" + $stdout) }; return $true }'
...
[void]$sign.Add($quoteFn)
[void]$sign.Add($invokeFn)
...
[void]$ver.Add($quoteFn)
[void]$ver.Add($invokeFn)
