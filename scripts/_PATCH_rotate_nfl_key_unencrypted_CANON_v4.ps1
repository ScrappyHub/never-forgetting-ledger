...
function Invoke-SshKeygenFiles([string[]]$Argv,[int]$TimeoutSeconds){
  ...
  try {
    # IMPORTANT: pass ArgumentList as ARRAY (preserves empty-string args like -N "")
    ...
    if (-not $ok) {
      ...
      Die ("ssh-keygen timeout after " + $TimeoutSeconds + "s`nARGV: " + (($Argv | ForEach-Object { if ($_ -eq "") { '""' } else { $_ } }) -join " ") + "`nSTDERR:`n" + $se + "`nSTDOUT:`n" + $so)
    }
    ...
    if ($p.ExitCode -ne 0) {
      Die ("ssh-keygen failed (exit " + $p.ExitCode + ")`nARGV: " + (($Argv | ForEach-Object { if ($_ -eq "") { '""' } else { $_ } }) -join " ") + "`nSTDERR:`n" + $stderr + "`nSTDOUT:`n" + $stdout)
    }
...
# Generate new UNENCRYPTED key (NO prompts) using ArgumentList array
