...
# ----------------------------
# Rewrite sign_file_v1.ps1 (Write-Output only)
# ----------------------------
...
# ----------------------------
# Rewrite verify_sig_v1.ps1 (Write-Output only + explicit verify marker)
# ----------------------------
...
if (`$DebugFindPrincipals) {
  ...
  if ([string]::IsNullOrWhiteSpace(`$out)) {
    Write-Output "DEBUG: find-principals produced EMPTY output"
  } else {
    Write-Output "DEBUG: find-principals output:"
    Write-Output `$out
  }
}
...
# ----------------------------
# Rewrite debug_verify_find_principals_v1.ps1 (Write-Output only)
# ----------------------------
...
# ----------------------------
# Re-sign + verify (leave as-is; now child scripts emit on stdout)
# ----------------------------
...
& $PSExe -NoProfile -ExecutionPolicy Bypass -File $Sign -File $Tb -Namespace "nfl/ingest-receipt" -TimeoutSeconds 120
& $PSExe -NoProfile -ExecutionPolicy Bypass -File $Dbg  -File $Tb -Sig $Sig -Namespace "nfl/ingest-receipt" -TimeoutSeconds 120
& $PSExe -NoProfile -ExecutionPolicy Bypass -File $Ver  -File $Tb -Sig $Sig -Namespace "nfl/ingest-receipt" -TimeoutSeconds 120 -DebugFindPrincipals
