...
# Backup
$bak = $Lib + ".bak_" + (Get-Date).ToString("yyyyMMdd_HHmmss")
Copy-Item -LiteralPath $Lib -Destination $bak -Force
"BACKUP_LIB: $bak" | Out-Host

# Rewrite lib with PS 5.1-safe ProcessStartInfo runner (no ArgumentList property)
$libText = @'
$ErrorActionPreference="Stop"
Set-StrictMode -Version Latest
...
Set-Content -LiteralPath $Lib -Value $libText -Encoding UTF8
[ScriptBlock]::Create((Get-Content -Raw -LiteralPath $Lib)) | Out-Null
"WROTE: $Lib" | Out-Host

"OK: lib patched to ProcessStartInfo runner" | Out-Host
