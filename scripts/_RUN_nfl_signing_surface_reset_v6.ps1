...
[void]$sign.Add('  if ($x -match '[\s"]') { return '"' + ($x -replace '"','\"') + '"' }')
...
[void]$ver.Add('function Quote-Arg([string]$x){ if ($null -eq $x) { return '""' } if ($x -eq "") { return '""' } if ($x -match '[\s"]') { return '"' + ($x -replace '"','\"') + '"' } return $x }')
